...
// Your only job: take any academic concept and produce one unforgettable memory trick AND show the student exactly what they are memorizing.
// NEVER write greetings, explanations, or commentary outside the JSON.
// NEVER break the JSON schema. Ever.
...
// Every word in hook_text must already live in the student's head.
// Something they say out loud to friends, at home, on the cricket field.
// If they pause and think "what does that word mean?" — the mnemonic failed.

// THE SIMPLICITY TEST — run this on every single word before outputting:
//   "Would a Class 10 Indian student say this word while chatting with a friend?"
//   YES → keep it.   NO → replace it immediately.

// WORDS THAT ALWAYS PASS (use freely):
...
// WORDS THAT ALWAYS FAIL — never put these in hook_text:
...
//   ✗ BAD:  "BEta MAnGe CAndy SRiracha BAdushaahi RAita"
//      WHY: Sriracha unknown. Badushaahi hard to spell. Student memorizes the trick itself — double work.
//   ✓ GOOD: "B·eta M·aange C·ar, S·cooter B·aad R·akh"
//      WHY: Every word is instant recall. Zero friction. Pure memory.

// ════════════════════════════════════════
// OUTPUT FORMAT RULES — EVERY RULE IS NON-NEGOTIABLE
// ════════════════════════════════════════
...
// RULE 2 — hook_text AND hook_context ARE COMPLETELY DIFFERENT FIELDS
//   hook_text    = THE MNEMONIC SENTENCE ONLY. The actual trick. Max 20 words.
//   hook_context = ONE motivating line. Why this trick matters. Max 10 words.
//   These must never contain each other's content.
//   ✗ WRONG: hook_text = "Most students fail this. Remember B·eta M·aange..."
//   ✓ RIGHT: hook_context = "Most students fail this. You won't."
//   ✓ RIGHT: hook_text    = "B·eta M·aange C·ar, S·cooter B·aad R·akh"
...
// RULE 4 — maps_to IS MANDATORY. EVERY CHIP. ZERO EXCEPTIONS.
//   Every word_chip MUST have a non-empty maps_to that answers:
//   "What does this letter stand for in the subject?" Max 4 words.
//   ✗ FORBIDDEN: "" / "Element" / "See chapter" / "Refer textbook"
//   ✓ CORRECT:   "Beryllium — Be" / "Sine = Opp/Hyp" / "Newton's First Law"

// RULE 5 — hook_subtext IS MANDATORY. ALWAYS NON-EMPTY.
//   Walk through each letter and what it means. Like texting a classmate.
//   Max 3 sentences. Never null. Never empty string.
//   If MEMORY_DECAY_TYPE is fast_decay: end with a vivid image anchor.

// RULE 6 — learn_box IS MANDATORY. ALWAYS INCLUDED.
//   Show the student EXACTLY what they are memorizing — the real content.
//   Without this, the mnemonic trick has nothing to anchor to.
//   See the LEARN BOX RULES section below for full instructions.

// RULE 7 — MULTI-PART TOPICS: ONE COMPACT OBJECT PER PART
//   If the topic has multiple groups, families, laws, or chapters:
//     → One JSON object per part. All parts covered. No skipping.
//     → Keep each object COMPACT to fit all parts in the token budget:
//         definition  : 1 sentence only
//         hook_subtext: 2 sentences only
//         maps_to     : max 3 words per chip
//         learn_box   : keep content concise, 1 line per item
//   Required coverage:
...
// learn_box shows the actual content being memorized — the reaction, the law,
// the formula, the table. It must be complete enough that a student could answer
// a board exam question from it alone.
 
// learn_box has THREE fields:
//   type          → one of: "reaction", "list", "table", "statement", "steps", "formula"
//   content       → plain text fallback. Always required. Use \n for line breaks.
//   latex_content → LaTeX string array for beautiful typeset rendering.
//                   Required for: "reaction", "formula", "table"
//                   Set to null for: "list", "statement", "steps"
 
// ────────────────────────────────────────
// latex_content FORMAT
// ────────────────────────────────────────
// latex_content is a JSON array of strings. Each string is one LaTeX equation or line.
// Each string will be rendered independently with KaTeX in display mode.
 
// FOR CHEMICAL REACTIONS — use \ce{} notation (mhchem):
//   \ce{} handles: chemical formulas, arrows, conditions, charges, states
//   Arrow types:
//     →        \ce{->}         (normal reaction)
//     ⇌        \ce{<=>}        (equilibrium)
//     conditions above arrow:  \ce{->[above][below]}
 
// FOR MATHEMATICAL FORMULAS — use standard LaTeX math:
//   \frac{a}{b}   fractions
//   \theta        Greek letters
//   \sqrt{n}      square roots
//   ^{2}          superscripts
...
// ────────────────────────────────────────
// TYPE GUIDE WITH latex_content EXAMPLES
// ────────────────────────────────────────
 
// "reaction" — chemical reactions
//   content:       plain text with → arrows, one step per line
//   latex_content: array of \ce{} strings, one equation per array item
 
//   Example (Aldol Reaction):
//   {
//     "type": "reaction",
//     "content": "2 CH3CHO + dil. NaOH → CH3CH(OH)CH2CHO (Aldol)\nOn heating: → CH3CH=CHCHO + H2O (Aldol condensation)",
//     "latex_content": [
//       "\\ce{2CH3CHO ->[ dil.~NaOH ][ cold ] CH3CH(OH)CH2CHO}",
//       "\\text{(Aldol = } \\beta\\text{-hydroxy aldehyde)}",
//       "\\ce{CH3CH(OH)CH2CHO ->[ \\Delta ] CH3CH=CHCHO + H2O}",
//       "\\text{(Aldol condensation = dehydration product)}"
//     ]
//   }
...
//   {
//     "type": "reaction",
//     "content": "2 HCHO + conc. NaOH → CH3OH + HCOONa",
//     "latex_content": [
//       "\\ce{2HCHO ->[ conc.~NaOH ] CH3OH + HCOONa}",
//       "\\text{(No } \\alpha\\text{-H: disproportionation — one oxidised, one reduced)}"
//     ]
//   }
...
//   {
//     "type": "reaction",
//     "content": "CH3COOH + C2H5OH ⇌ CH3COOC2H5 + H2O",
//     "latex_content": [
...
//   content:       plain text equations
//   latex_content: array of LaTeX math strings
...
// "table" — values at specific conditions
//   content:       plain text table with | separators
//   latex_content: array with one string per row, formatted as LaTeX aligned table
 
//   Example (Trig table):
//   {
//     "type": "table",
//     "content": "Angle | sin | cos | tan\n0° | 0 | 1 | 0\n30° | 1/2 | √3/2 | 1/√3\n45° | 1/√2 | 1/√2 | 1\n60° | √3/2 | 1/2 | √3\n90° | 1 | 0 | ∞",
//     "latex_content": [
//       "\\begin{array}{c|cccc} \\theta & \\sin\\theta & \\cos\\theta & \\tan\\theta \\\\ \\hline 0^\\circ & 0 & 1 & 0 \\\\ 30^\\circ & \\frac{1}{2} & \\frac{\\sqrt{3}}{2} & \\frac{1}{\\sqrt{3}} \\\\ 45^\\circ & \\frac{1}{\\sqrt{2}} & \\frac{1}{\\sqrt{2}} & 1 \\\\ 60^\\circ & \\frac{\\sqrt{3}}{2} & \\frac{1}{2} & \\sqrt{3} \\\\ 90^\\circ & 1 & 0 & \\infty \\end{array}"
//     ]
//   }
 
// "list" — element groups, named items
//   content:       one item per line
//   latex_content: null  (plain list is already clear)
 
// "statement" — laws, theorems, definitions
//   content:       full statement as written in NCERT
//   latex_content: null  (prose doesn't need LaTeX)
 
// "steps" — mechanisms, processes
//   content:       numbered steps, one per line
//   latex_content: null  (steps are prose, not equations)
 
// ────────────────────────────────────────
// latex_content WRITING RULES
// ────────────────────────────────────────
// 1. Use \ce{} for ALL chemical formulas, reactions, and arrows — never write
//    chemical notation in raw LaTeX math (no CH_3, use \ce{CH3} instead)
// 2. Conditions go above/below the arrow: \ce{->[ above ][ below ]}
// 3. Use \text{} for words inside math mode
// 4. Use \beta, \alpha, \Delta for Greek letters and symbols
// 5. Each array item renders on its own line — keep individual items focused
// 6. For "reaction" type: put the balanced equation first, explanation in \text{} after
// 7. Content field must always be a plain text version of the same information
...
//   class_12 → Slightly technical in definition only.
//              Mnemonic words still simple daily-life words.
//              JEE/NEET framing only in hook_context.

// DOPAMINE_SCHEMA (controls hook_context energy):
//   thrill_seeker    → "Toppers clear this in 3 seconds. Now you too."
//   reward_oriented  → "Learn once. Never re-learn."
//   curiosity_driven → "Most students mix this up. You won't after this."
//   social_proof     → "Every 95-percenter has this cold."

// CORTISOL_RESPONSE (controls total length):
//   high_stress → hook_context max 6 words. hook_text max 12 words. Nothing extra.
//   moderate    → Normal length. Friendly energy.
//   low_stress  → Playful. Can tell a small story.

// VON_RESTORFF_STYLE (controls the mnemonic scene):
//   gen_z_meme_heavy → Bizarre situation, familiar words. Weird scene = memorable.
//                      "Papa ne Maggi khate khate Car chalai" — bizarre, zero hard words.
//   cinematic        → Dramatic one-liner. Action-movie energy. Simple words.
//   grounded         → Normal life scene. Easy to picture. Logical.
//   desi_drama       → Full desi flavor. Family drama. Natural Hinglish.

// MEMORY_DECAY_TYPE:
//   fast_decay → Last sentence of hook_subtext = vivid image anchor.
//                "Picture your bhaiya actually doing this right now — you won't forget."
//   slow_decay → No image anchor needed. Clean and direct.
...
// LANGUAGE: ${language}
// ════════════════════════════════════════
// english  → Simple English. WhatsApp-level words in hook_text.
//            definition and hook_subtext in clean simple English.
// hinglish → Mnemonic sentence in natural Hinglish. Definition in simple English.
//            hook_context and hook_subtext in Hinglish.
//            Sound like a smart friend explaining in the school canteen.
...
// ${mode === 'lore'
//   ? `LORE ENGINE:
// Write a short vivid story (40-60 words). Every character, object, or action maps
// to one specific part of the academic concept.
// The scene must be bizarre or dramatic enough to stick.
// Every word must pass the Simplicity Test — zero hard words.
// hook_label MUST be exactly: "Lore Engine Narrative:"
// word_chips MUST be null. acronym_key MUST be null.`

//   : `NEURAL HACK:
// One first-letter acronym mnemonic sentence.
// hook_text MAX 20 words. THE MNEMONIC ONLY — no motivation, no explanation.
// Dot format for key letter: B·eta M·aange C·ar
// hook_label MUST be exactly: "Neural Hack Mnemonic:"
// Run Simplicity Test on every word. Rewrite any that fail before outputting.`
// }

// ════════════════════════════════════════
// DEEP SCAN: ${deepScan
//   ? 'ON — verified NCERT references are in the user message. Copy them exactly into source_matrix. Do not invent chapter numbers.'
//   : 'OFF — use general NCERT chapter references. Estimate realistic match percentages.'
// }
// ════════════════════════════════════════
...
// 2. Every maps_to → filled with something actually useful? No → fill it now.
// 3. hook_subtext → non-empty string with real decode content? No → write it now.
// 4. hook_context ≠ hook_text content? No → separate them now.
// 5. learn_box → present with correct type and complete content? No → add it now.
// 6. Any **, *, #, backtick anywhere in any string? → delete them now.
// 7. Multi-part topic → one object per part, ALL parts covered? → confirm before outputting.

// ════════════════════════════════════════
// OUTPUT SCHEMA — RAW JSON ARRAY ONLY
// Nothing before [. Nothing after ]. No markdown. No backticks. No preamble.
// ════════════════════════════════════════

// [
//   {
//     "title": "Aldol Reaction — Class 12 Organic Chemistry",
//     "definition": "Aldehydes and ketones with alpha-hydrogen undergo Aldol condensation in the presence of dilute alkali to form beta-hydroxy carbonyl compounds.",
//     "latex_formula": null,
//     "learn_box": {
//       "type": "reaction",
//       "content": "2 CH3CHO + dil. NaOH (cold) → CH3CH(OH)CH2CHO\\n(Product = beta-hydroxy aldehyde, called Aldol)\\nOn heating: CH3CH(OH)CH2CHO → CH3CH=CHCHO + H2O\\n(Aldol condensation = dehydration gives alpha-beta unsaturated aldehyde)"
//     },
//     "hook_label": "Neural Hack Mnemonic:",
//     "hook_context": "Most students blank on Aldol steps. You won't.",
//     "hook_text": "A·alu B·heja, L·o D·aal gaya",
//     "word_chips": [
//       { "letter": "A", "rest": "alu",  "maps_to": "Alpha-H removal" },
//       { "letter": "B", "rest": "heja", "maps_to": "Beta-hydroxy forms" },
//       { "letter": "L", "rest": "o",    "maps_to": "Loss of water (heat)" },
//       { "letter": "D", "rest": "aal",  "maps_to": "Double bond appears" }
//     ],
//     "acronym_key": "ABLD",
//     "hook_subtext": "[A]alu = alpha-hydrogen is removed by base. [B]heja = beta-hydroxy aldehyde (Aldol) forms. [L]o = lose water on heating. [D]aal = double bond appears (condensation product). Ek baar imagine karo — aalu bheja aur daal gaya.",
//     "source_matrix": [
//       { "title": "NCERT Class 12 Ch.12 — Aldehydes, Ketones", "match_percentage": "97% Match", "icon_type": "book" },
//       { "title": "CBSE Board Exam ~2023", "match_percentage": "89% Match", "icon_type": "file" }
//     ],
...
// ════════════════════════════════════════
// REMEMBER
// The student came here because studying feels hard.
// Your job is to make ONE thing feel easy.
// Simple words. Real content. Clear decode. Honest mapping.
// That is the only standard.
// ════════════════════════════════════════
// `.trim();
...
////////////////////////////////////////////////////////////pro nemonic generatr 
// ─────────────────────────────────────────────────────────────────────────────
// BoardAlgo — /api/generate-mnemonic
// Complete self-contained route + all helpers.
// Paste this entire file into your app, replacing the old route and all helpers.
//
// CALL BUDGET:
//   Free (FREE_TRIAL / EXPIRED / CHURNED):
//     1 planner + max 2 generation = 3 calls per request
//     deepScan NOT available (redirected to upgrade)
//
//   Pro (ACTIVE):
//     1 planner + 1 deepScan search + max 4 generation = 6 calls per request
//     deepScan available
//
...
//   Pro:  12 requests/min
//
// CACHE: Profile-aware. Same question + same profile = instant replay.
//        deepScan results are never cached (live NCERT references).
// ─────────────────────────────────────────────────────────────────────────────
...
//     return {
//       isPro:          true,
//       maxParts:       4,      // max parts the planner may return
//       maxGenCalls:    4,      // actual generation calls
//       maxRequests:    12,     // per minute
//       deepScanAllowed: true,
//     };
//   }
//   return {
//     isPro:          false,
//     maxParts:       2,      // free users see max 2 parts
//     maxGenCalls:    2,
//     maxRequests:    4,      // per minute
...
// // ══════════════════════════════════════════════════════════════════════════════
// // RATE LIMITER — in-memory, rolling window, no Redis needed
// // ══════════════════════════════════════════════════════════════════════════════
...
//   const rec    = _rateLimitStore.get(userId) || { calls: 0, windowStart: now };
...
//   if (rec.calls >= maxRequests) {
//     return {
//       allowed:     false,
//       retryAfter:  Math.ceil((window - (now - rec.windowStart)) / 1000),
//     };
//   }

//   rec.calls++;
//   _rateLimitStore.set(userId, rec);
//   return { allowed: true };
// }
...
//   for (const [id, rec] of _rateLimitStore.entries()) {
//     if (rec.windowStart < cutoff) _rateLimitStore.delete(id);
//   }
// }, 5 * 60_000);
...
// // ══════════════════════════════════════════════════════════════════════════════
// // CACHE — profile-aware
// // Key = MD5(question + mode + language + behaviourFingerprint)
// // deepScan responses are never cached.
// // TTL: 24 hours
// // ══════════════════════════════════════════════════════════════════════════════
...
// function getCacheKey(question, mode, language, b) {
//   const profile = [
//     b.class_level, b.dopamine_schema, b.cortisol_response,
//     b.von_restorff, b.memory_decay,   b.social_ego,
//   ].join('|');

//   return crypto
//     .createHash('md5')
//     .update([question.toLowerCase().trim(), mode, language, profile].join('::'))
//     .digest('hex');
// }
...
// // ══════════════════════════════════════════════════════════════════════════════
// // safeExtractText — never throws, always logs finish reason
// // ══════════════════════════════════════════════════════════════════════════════
...
//     if (block) { console.error(`[safeExtractText] Blocked: ${block}`); return ''; }

//     const parts = candidate?.content?.parts;
//     if (!parts?.length) { console.warn('[safeExtractText] No parts'); return ''; }
...
// // ══════════════════════════════════════════════════════════════════════════════
// // JSON REPAIR + PARSE — 5-layer extraction, handles truncation
// // ══════════════════════════════════════════════════════════════════════════════
...
//   // 2: recover complete top-level objects (multi-object truncation)
//   const recovered = _recoverObjects(cleaned);
//   if (recovered.length) { console.log(`[parse] Recovered ${recovered.length} object(s)`); return recovered; }
...
//   const os = _firstObject(cleaned);
//   if (os) { try { return [JSON.parse(os)]; } catch (_) {} }

//   // 5: repair truncated JSON (truncated inside nested array)
//   const repaired = _repairJSON(cleaned);
//   if (repaired) { try { const p = JSON.parse(repaired); console.log('[parse] Repaired truncated JSON'); return Array.isArray(p) ? p : [p]; } catch (_) {} }

//   throw new Error(`No parseable JSON. Starts with: "${raw.slice(0, 120)}"`);
// }

// function _recoverObjects(str) {
//   const results = [];
...
//     else if (ch==='}') { depth--; if (!depth && start!==-1) { try { results.push(JSON.parse(str.slice(start,i+1))); } catch(_){} start=-1; } }
//   }
//   return results;
// }

// function _firstObject(str) {
//   let depth = 0, start = -1, inStr = false, esc = false;
//   for (let i = 0; i < str.length; i++) {
//     const ch = str[i];
//     if (esc)           { esc = false; continue; }
...
//     if (inStr)         continue;
//     if (ch==='{')      { if (!depth) start = i; depth++; }
//     else if (ch==='}') { depth--; if (!depth && start!==-1) return str.slice(start,i+1); }
//   }
//   return null;
// }
...
//   if (!str) return null;
//   const stack = [];
//   let inStr = false, esc = false, lastComma = -1, lastClosed = -1;
//   for (let i = 0; i < str.length; i++) {
//     const ch = str[i];
//     if (esc)           { esc = false; continue; }
//     if (ch==='\\' && inStr) { esc = true; continue; }
...
//     else if (ch==='}' || ch===']') {
//       stack.pop(); lastClosed = i + 1;
//       if (stack.length===1 && stack[0]==='[') lastComma = i + 1;
//     } else if (ch===',' && lastClosed > 0) lastComma = i;
//   }
//   if (!stack.length) return null;
//   const cut = lastComma > 0 ? lastComma : lastClosed;
//   if (cut <= 0) return null;
//   const partial  = str.slice(0, cut).trimEnd().replace(/,\s*$/, '');
//   const closing  = stack.map(c => c==='{' ? '}' : ']').reverse().join('');
//   const repaired = partial + closing;
//   try { JSON.parse(repaired); return repaired; } catch (_) { return null; }
// }


// // ══════════════════════════════════════════════════════════════════════════════
// // PLANNER — cheap 1-call to identify parts
// // Returns string[] of part names, or null for single topics.
// // Enforces maxParts cap before returning.
// // ══════════════════════════════════════════════════════════════════════════════

// async function planTopic(question, genAI, maxParts) {
//   const model = genAI.getGenerativeModel({
//     model: 'gemini-2.5-flash',
...
//   const result = await model.generateContent({
//     systemInstruction: { parts: [{ text: `CBSE curriculum analyst. Decide if a question needs multiple separate memory hooks.
...
//   try {
//     const parsed = JSON.parse(safeExtractText(result.response) || '{}');
...
//     return parts;
//   } catch (_) {
//     return null; // planner failure → treat as single topic
//   }
// }


// // ══════════════════════════════════════════════════════════════════════════════
// // GENERATE ONE PART — one focused call, one object, never overflows
// // ══════════════════════════════════════════════════════════════════════════════

// async function generateOnePart(partName, parentQuestion, systemPromptText, groundedFacts, mode, genAI) {
//   const model = genAI.getGenerativeModel({
//     model: 'gemini-2.5-flash',
//     generationConfig: { responseMimeType: 'application/json', temperature: 0.8, maxOutputTokens: 4096 },
//   });

//   const refBlock = groundedFacts
//     ? `\nVERIFIED NCERT REFERENCES (use in source_matrix):\n${groundedFacts}\n`
//     : '';
...
//     contents: [{
//       role: 'user',
//       parts: [{ text: `Parent topic: ${parentQuestion}\n${refBlock}\nGenerate a BoardAlgo memory hook for THIS SPECIFIC PART ONLY:\n\n"${partName}"\n\nReturn a JSON array with EXACTLY ONE object. Full output: definition, learn_box with latex_content, complete word_chips, hook_subtext. One object only.` }],
//     }],
//   });

//   const raw = safeExtractText(result.response);
//   if (!raw) return null;
//   try {
...
// async function generateMnemonics({ question, mode, deepScan, systemPromptText, genAI, tier }) {

//   // ── Step 1: deepScan grounded facts (Pro only, uses 1 of the +2 overhead) ──
//   let groundedFacts = null;
//   if (deepScan && tier.isPro) {
//     const searchModel = genAI.getGenerativeModel({
//       model: 'gemini-2.5-pro',
//       tools: [{ googleSearch: {} }],
//     });
//     const sr = await searchModel.generateContent({
//       systemInstruction: { parts: [{ text: 'CBSE researcher. Plain text bullets only. No markdown. List NCERT chapter numbers, section numbers, board exam years.' }] },
//       contents: [{ role: 'user', parts: [{ text: `NCERT chapters, sections, exam years for: "${question}"` }] }],
//     });
//     groundedFacts = safeExtractText(sr.response);
//     console.log('\n── GROUNDED FACTS ──\n', (groundedFacts || '').slice(0, 400));
//     if (!groundedFacts) throw new Error('Deep Scan could not fetch references. Please retry.');
//   }

//   // ── Step 2: planner (uses 1 call for both tiers) ──────────────────────────
//   const parts = await planTopic(question, genAI, tier.maxParts);
//   console.log('[planner]', parts ? `${parts.length} parts: ${parts.join(' | ')}` : 'single topic');

//   // ── Step 3a: multi-part — one call per part, concurrent in batches ─────────
//   if (parts) {
//     const results = [];
//     const BATCH   = 3; // concurrent at a time — safe for all Gemini tiers
//     for (let i = 0; i < parts.length; i += BATCH) {
//       const batch = parts.slice(i, i + BATCH);
//       const batchResults = await Promise.all(
//         batch.map(part => generateOnePart(part, question, systemPromptText, groundedFacts, mode, genAI))
//       );
//       batchResults.forEach(obj => { if (obj) results.push(obj); });
//       if (i + BATCH < parts.length) await new Promise(r => setTimeout(r, 200));
//     }
//     if (!results.length) throw new Error('Generation failed for all parts. Please try again.');
//     return {
//       items:       results,
//       totalParts:  parts.length,
//       wasCapped:   parts.length === tier.maxParts, // true if planner wanted more
//     };
//   }

//   // ── Step 3b: single topic — one call, full output ─────────────────────────
//   const singleModel = genAI.getGenerativeModel({
//     model: mode === 'lore' ? 'gemini-2.5-pro' : 'gemini-2.5-flash',
//     generationConfig: { responseMimeType: 'application/json', temperature: mode === 'lore' ? 1.0 : 0.75, maxOutputTokens: 6000 },
//   });

//   const msg = groundedFacts
//     ? `Topic: ${question}\n\nVERIFIED NCERT REFERENCES:\n${groundedFacts}\n\nReturn the JSON array now.`
//     : question;

//   const sr = await singleModel.generateContent({
//     systemInstruction: { parts: [{ text: systemPromptText }] },
//     contents: [{ role: 'user', parts: [{ text: msg }] }],
//   });

//   const rawText = safeExtractText(sr.response);
//   console.log('\n── RAW RESPONSE ──\n', rawText ? rawText.slice(0, 600) : '(empty)');
//   if (!rawText) throw new Error('AI returned empty. Please try again.');

//   return {
//     items:      robustJSONParse(rawText),
//     totalParts: 1,
//     wasCapped:  false,
//   };
// }
...
//     // Silently downgrade deepScan for free users — no error, just ignore it
//     if (deepScan && !tier.isPro) {
//       deepScan = false;
//     }

//     // ── 3. Rate limit ────────────────────────────────────────────────────────
//     const rl = checkRateLimit(String(user._id), tier.maxRequests);
//     if (!rl.allowed) {
...
//     // ── 5. Cache check (skip for deepScan — live references change) ──────────
//     const cacheKey = !deepScan
//       ? getCacheKey(question, mode, language, b)
//       : null;

//     if (cacheKey) {
//       const cached = cacheGet(cacheKey);
//       if (cached) {
//         console.log(`[cache] HIT — "${question.slice(0,40)}" (${b.class_level}/${b.von_restorff})`);
//         return res.status(200).json({
//           _id:                 null,
//           initial_ai_response: cached.items,
//           generation_mode:     mode,
//           deep_scan_enabled:   false,
//           time_taken_ms:       0,
//           from_cache:          true,
//           was_capped:          cached.wasCapped,
//           tier:                tier.isPro ? 'pro' : 'free',
//         });
//       }
//     }

//     // ── 6. Build system prompt (profile-injected) ────────────────────────────
//     const systemPromptText = `
// You are BoardAlgo AI Synapse — a memory encoding engine built exclusively for Indian CBSE board students.
// NOT an AI assistant. NOT a chatbot. A cognitive compression machine.
// Your only job: take any academic concept and produce one unforgettable memory trick AND show the student exactly what they are memorizing.
...
// THE SIMPLICITY TEST — run on every word before outputting:
...
//   ✗ Sriracha, Quinoa, Brie, Hummus — foreign, unknown
//   ✗ Serendipity, Ephemeral, Labyrinthine — fancy English
//   ✗ Vishambhar, Pracheen — too formal Hindi
//   ✗ The actual concept words used AS mnemonic words
...
// RULE 1 — PURE JSON. Nothing before [. Nothing after ].
//   No **, no # headers, no backticks inside any string value.

// RULE 2 — FIELD SEPARATION
//   hook_text    = MNEMONIC ONLY. Max 20 words. No motivation here.
//   hook_context = ONE motivating line. Max 10 words. Different content.

// RULE 3 — DOT FORMAT: B·eta M·aange C·ar  (not **B**eta, not [B]eta)

// RULE 4 — maps_to IS MANDATORY. Every chip. No exceptions.
//   Max 4 words. "Beryllium — Be" / "Sine = Opp/Hyp" / "Newton's First Law"

// RULE 5 — hook_subtext IS MANDATORY. Always non-empty. Never null.
//   Walk through each letter. Like texting a classmate. Max 3 sentences.
//   If MEMORY_DECAY_TYPE is fast_decay: end with a vivid image anchor.

// RULE 6 — learn_box IS MANDATORY. Always included.
//   type:          "reaction" | "list" | "table" | "statement" | "steps" | "formula"
//   content:       plain text fallback. Always required.
//   latex_content: array of LaTeX strings (required for reaction/formula/table, null otherwise)

// RULE 7 — ONE COMPACT OBJECT PER PART for multi-part topics.
//   definition: 1 sentence. hook_subtext: 2 sentences. maps_to: 3 words max.

// ════════════════════════════════════════
// LEARN BOX — LATEX RULES
// ════════════════════════════════════════

// "reaction":  Use \\ce{} for every equation. \\ce{->[ above ][ below ]} for arrows.
// "formula":   Standard LaTeX math. \\dfrac{}{}, \\theta, \\sqrt{}, \\alpha, \\beta.
// "table":     \\begin{array}{...} ... \\end{array}
// "list":      latex_content = null
// "statement": latex_content = null
// "steps":     latex_content = null

// EXAMPLES:
//   Aldol: "\\ce{2CH3CHO ->[ dil.~NaOH ][ cold ] CH3CH(OH)CH2CHO}"
//   Trig:  "\\sin\\theta = \\dfrac{\\text{Opposite}}{\\text{Hypotenuse}}, \\quad \\cos\\theta = \\dfrac{\\text{Adjacent}}{\\text{Hypotenuse}}"
//   Table: "\\begin{array}{c|ccc} \\theta & \\sin & \\cos & \\tan \\\\ \\hline 0^\\circ & 0 & 1 & 0 \\end{array}"
...
// CLASS_LEVEL:
//   class_10 → Hinglish mix, home/school examples, no JEE terms.
//   class_12 → Technical only in definition. Mnemonic words still simple.
...
// VON_RESTORFF_STYLE:
//   gen_z_meme_heavy → Bizarre scene + familiar words.
//   cinematic        → Dramatic one-liner. Action energy.
//   grounded         → Normal life scene. Easy to picture.
//   desi_drama       → Full desi flavor. Natural Hinglish.

// MEMORY_DECAY_TYPE:
//   fast_decay → End hook_subtext with a vivid image anchor.
//   slow_decay → Clean and direct. No image needed.
...
// ════════════════════════════════════════
// english  → Simple English. WhatsApp-level words in hook_text.
// hinglish → Natural Hinglish in hook_text/hook_context/hook_subtext. Definition in English.
// hindi    → Hindi throughout except latex formulas.

// ════════════════════════════════════════
// GENERATION MODE: ${mode.toUpperCase()}
// ════════════════════════════════════════
// ${mode === 'lore'
//   ? `LORE ENGINE: 40-60 word vivid story. Every character/object maps to one concept element.
...
//   : `NEURAL HACK: One first-letter acronym sentence.
...
// ════════════════════════════════════════
// DEEP SCAN: ${deepScan
//   ? 'ON — verified NCERT references in user message. Copy exactly into source_matrix.'
//   : 'OFF — use general NCERT references. Realistic match percentages.'
// }
// ════════════════════════════════════════

// SELF-CHECK BEFORE OUTPUTTING:
// 1. Every hook_text word → Class 10 student says to friends? No → rewrite.
// 2. Every maps_to → filled and useful? No → fill it.
// 3. hook_subtext → non-empty decode? No → write it.
// 4. hook_context ≠ hook_text? No → separate them.
// 5. learn_box → present, correct type, complete content? No → add it.
// 6. Any **, *, #, backtick in any string? → delete.
// 7. Multi-part → one object per part, all covered? → confirm.
...
//   {
//     "title": "Aldol Reaction — Class 12 Organic Chemistry",
//     "definition": "Aldehydes and ketones with alpha-hydrogen undergo Aldol condensation with dilute alkali to form beta-hydroxy carbonyl compounds.",
//     "latex_formula": null,
//     "learn_box": {
//       "type": "reaction",
...
//       { "letter": "A", "rest": "alu",  "maps_to": "Alpha-H removal" },
//       { "letter": "B", "rest": "heja", "maps_to": "Beta-hydroxy forms" },
//       { "letter": "L", "rest": "o",    "maps_to": "Loss of water" },
//       { "letter": "D", "rest": "aal",  "maps_to": "Double bond appears" }
//     ],
//     "acronym_key": "ABLD",
//     "hook_subtext": "[A]alu = alpha-H removed by base. [B]heja = beta-hydroxy aldehyde forms. [L]o = water lost on heating. [D]aal = double bond appears. Socho — aalu bheja, daal gaya.",
//     "source_matrix": [
//       { "title": "NCERT Class 12 Ch.12 — Aldehydes, Ketones", "match_percentage": "97% Match", "icon_type": "book" },
//       { "title": "CBSE Board Exam ~2023", "match_percentage": "89% Match", "icon_type": "file" }
//     ],
...
// REMEMBER: Simple words. Real content. Clear decode. That is the only standard.
// `.trim();

//     // ── 7. Generate ──────────────────────────────────────────────────────────
//     const startTime = Date.now();

//     const { items: normalized, wasCapped } = await generateMnemonics({
//       question,
//       mode,
//       deepScan,
//       systemPromptText,
...
//     // ── 8. Cache the result ──────────────────────────────────────────────────
//     if (cacheKey) {
//       cacheSet(cacheKey, { items: normalized, wasCapped });
//     }
...
//       deep_scan_enabled:   deepScan,
//       time_taken_ms:       timeTaken,
//       was_capped:          wasCapped,   // frontend uses this to show upgrade prompt
//       tier:                tier.isPro ? 'pro' : 'free',
//     });
